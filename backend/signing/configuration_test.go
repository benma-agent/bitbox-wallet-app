...
func TestBitcoinSimpleDescriptorScriptTypes(t *testing.T) {
	const testnetTPub = "tpubD6NzVbkrYhZ4XVPn5tSFsjZbYcAnoDizTifqiqFEU18GcLzJMMPeYkBL1tkPT94oxPpuaWeTrMnCoXqFcAwRUn83HeM9SSwZyeAg3J62ssn"

	xpub, err := hdkeychain.NewKeyFromString(testnetTPub)
	require.NoError(t, err)
	cfg := &BitcoinSimple{
		KeyInfo: KeyInfo{
	...
	tests := []struct {
		...
		{
			name:       "p2pkh",
			keypath:    mustKeypath("m/44'/1'/0'"),
			scriptType: ScriptTypeP2PKH,
			expectedDescriptor: "pkh([deadbeef/44'/1'/0']" +
				testnetTPub + "/<0;1>/*)#67jw2hwm",
		},
		{
			name:       "p2wpkh-p2sh",
			keypath:    mustKeypath("m/49'/1'/0'"),
			scriptType: ScriptTypeP2WPKHP2SH,
			expectedDescriptor: "sh(wpkh([deadbeef/49'/1'/0']" +
				testnetTPub + "/<0;1>/*))#pdregzl2",
		},
		{
			name:       "p2wpkh",
			keypath:    mustKeypath("m/84'/1'/0'"),
			...
			expectedDescriptor: "wpkh([deadbeef/84'/1'/0']" +
				testnetTPub + "/<0;1>/*)#8r4fxrzh",
		},
		{
			name:       "p2tr",
			keypath:    mustKeypath("m/86'/1'/0'"),
			scriptType: ScriptTypeP2TR,
			expectedDescriptor: "tr([deadbeef/86'/1'/0']" +
				testnetTPub + "/<0;1>/*)#7kheukj2",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			cfg.KeyInfo.AbsoluteKeypath = test.keypath
...
func TestDescriptorChecksumVector(t *testing.T) {
	...
	//
	// This can be reproduced manually by copy-pasting the script below:
	//
	//	package main
	//
	//	import descriptorsgo "github.com/benma/descriptors-go/descriptors"
	//
	//	func main() {
	//		d, _ := descriptorsgo.NewDescriptor("sh(multi(2,[00000000/111'/222]xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL,xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y/0))")
	//		println(d.String())
	//	}
